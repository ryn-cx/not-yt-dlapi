...
from not_yt_dlapi.exceptions import NoContentError, PlaylistItemsNotFoundError
...
def assert_playlist_items_not_found_error(
    endpoint: object,
    name: str,
    download: Callable[[], object],
) -> None:
    """Assert that download raises PlaylistItemsNotFoundError."""
    if data_path(endpoint, name).exists():
        pytest.skip(f"error already recorded for {type(endpoint).__name__}/{name}")
